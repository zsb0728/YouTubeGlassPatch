...
static UIVisualEffectView*YTGGlass(BOOL interactive) API_AVAILABLE(ios(26.0)){UIGlassEffect*e=[UIGlassEffect effectWithStyle:UIGlassEffectStyleRegular];e.interactive=interactive;UIVisualEffectView*v=[[UIVisualEffectView alloc]initWithEffect:e];v.userInteractionEnabled=NO;v.opaque=NO;v.backgroundColor=UIColor.clearColor;v.cornerConfiguration=[UICornerConfiguration capsuleConfiguration];return v;}
...
static void YTGStyle(UIView*host) API_AVAILABLE(ios(26.0)){
 ...
 CGFloat safe=host.safeAreaInsets.bottom;if(safe<1&&host.window)safe=host.window.safeAreaInsets.bottom;CGFloat h=MIN(58.0,MAX(48.0,host.bounds.size.height-safe-6.0));CGRect capsule=CGRectMake(16.0,MAX(2.0,(host.bounds.size.height-safe-h)/2.0),host.bounds.size.width-32.0,h);base.frame=capsule;base.hidden=NO;
 UIView*selected=items.firstObject;for(UIView*i in items)if(YTGSelected(i)){selected=i;break;}CGRect ir=[selected convertRect:selected.bounds toView:host];CGFloat lh=h-6.0;CGFloat x=MAX(CGRectGetMinX(capsule)+3,CGRectGetMinX(ir)+4),mx=MIN(CGRectGetMaxX(capsule)-3,CGRectGetMaxX(ir)-4);lens.frame=CGRectMake(x,CGRectGetMidY(capsule)-lh/2,MAX(28.0,mx-x),lh);lens.hidden=NO;
...
static void YTGPivotLayout(UIView*self,SEL cmd){if(gPivotLayoutIMP)((void(*)(id,SEL))gPivotLayoutIMP)(self,cmd);if(@available(iOS 26.0,*))YTGStyle(self);}
...
__attribute__((constructor(101)))static void YTGStart(void){YTGEnableFlags();dispatch_async(dispatch_get_main_queue(),^{YTGInstall();for(NSNumber*d in @[@0.05,@0.2,@0.5,@1.0,@2.0])dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(d.doubleValue*NSEC_PER_SEC)),dispatch_get_main_queue(),^{YTGEnableFlags();YTGInstall();});});}
